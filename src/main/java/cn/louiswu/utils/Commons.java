...
public class Commons {
     ...
     * @return
     */
    public static String fmtglwzdate(Date date) {
        if (null != date) {
    ...
    public static List<String> show_all_thumb(String content) {
        ...
        if (content.contains("<img")) {
            ...
            Matcher m_image = p_image.matcher(content);
            if (m_image.find()){
                String data = m_image.group(1);
                String[] datas = data.split("\r|\n");
                for (int i = 0;i<datas.length;i++){
                    if(!"".equals(datas[i]) && datas[i].contains("<img")) {
                        String imgData =  datas[i].split("\"")[1];
                        rs.add(imgData);
                    }
...
//            while (m_image.find()) {
//                String data = m_image.group(1).trim().replaceAll("\r|\n", "");;
